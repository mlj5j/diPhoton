...
for j_entry in range(entries):
    ...
    #----This 'if' statement needs to be changed from hlt to met filters------    
...
#print "phtons id is {0}".format(bool(ch.Photons_fullID[i_photon]))
            if bool(ch.Photons_fullID[i_photon]) and bool(check_iso(ch.Photons[i_photon],ch.Jetsclean)):
        ...
        for i_jet in range(0,ch.Jetsclean.size()):
            min_dR[0] = find_deltaR(ch.Jetsclean[i_jet], ch.Photons)
            if (ch.Jetsclean[i_jet].Pt()>30) and (abs(ch.Jetsclean[i_jet].Eta())<2.4) and (min_dR[0]>0.4):
                ST[0] += ch.Jetsclean[i_jet].Pt()
                ST_jets[0] += ch.Jetsclean[i_jet].Pt()
                vJets += ch.Jetsclean[i_jet]
                vHardMET -= ch.Jetsclean[i_jet]

        for i_jet in range(0,ch.Jetsclean.size()):
            if ch.Jetsclean[i_jet].Pt()>30 and abs(ch.Jetsclean[i_jet].Eta())<2.4 and min_dR[0]>0.4:
                DPhiHardMetJets = abs(ch.Jetsclean[i_jet].DeltaPhi(vHardMET))
#                print "Made it into jet thingy"
                
                if DPhiHardMetJets < MinDPhiHardMetJets[0]:
                    MinDPhiHardMetJets[0] = DPhiHardMetJets
                    MinDPhiJetPt[0] = ch.Jetsclean[i_jet].Pt()
